...
app_MockFlock.prototype = {
	init: function() {
		...
		this.listen.clickSign.handle(function(mouse) {
			console.log("app/MockFlock.hx:48:","inside rect " + Std.string(postite_geom_GeomFilters.isInsideRect(postite_geom__$CoolPoint_CoolPoint_$Impl_$.toPoint(mouse),_gthis.rect)));
			console.log("app/MockFlock.hx:55:","pulo=" + Std.string(postite_geom__$PolyGon_PolyGon_$Impl_$.contains(_gthis.hullPol,postite_geom__$CoolPoint_CoolPoint_$Impl_$.toPoint(mouse),true)));
			return;
		});
		this.listen.pointSign.handle(function(cpoint) {
			console.log("app/MockFlock.hx:58:","oui ma point " + Std.string(cpoint));
			return;
		});
		var _g2 = 0;
	...
	,update: function() {
		var _this = app_MockFlock.coolPoints;
		...
		while(_g < _g1) {
			var i = _g++;
			var cp = _this[i];
			result[i] = { x : cp.x + postite_math_Matools.getRandomInt(-5,5), y : cp.y + postite_math_Matools.getRandomInt(-5,5), press : cp.press};
		}
		app_MockFlock.coolPoints = result;
		var _this1 = app_MockFlock.coolPoints;
		var result1 = new Array(_this1.length);
		var _g2 = 0;
		var _g11 = _this1.length;
		while(_g2 < _g11) {
			var i1 = _g2++;
			result1[i1] = postite_geom__$CoolPoint_CoolPoint_$Impl_$.toPoint(_this1[i1]);
		}
	...
	,render: function(_render) {
		...
		while(_g < _g1) {
			var i = _g++;
			var a = path[i];
			result[i] = { x : a.x, y : a.y};
		}
		var pathPoints = result;
		var sim1 = postite_geom_GeomFilters.isClosedAt(pathPoints);
		postite_dro_Dro.drawPoly(_render.ctx,pathPoints);
		switch(sim1._hx_index) {
		case 0:
			var _g2 = sim1.v;
			postite_dro_Dro.droPoint(_render.ctx,postite_geom__$CoolPoint_CoolPoint_$Impl_$._new(_g2.x,_g2.y,10),postite_dro__$Couleur_Couleur_$Impl_$.fromString("#00AAFF"));
			break;
		case 1:
			break;
		}
		var _this = postite_dro_Coords.closed;
		...
		while(_g3 < _g11) {
			var i1 = _g3++;
			var a1 = _this[i1];
			result1[i1] = { x : a1.x, y : a1.y};
		}
		var sim = postite_geom_GeomFilters.isClosedAt(result1);
		postite_dro_Dro.drawPoly(_render.ctx,result1);
		switch(sim._hx_index) {
		case 0:
			var _g12 = sim.v;
			postite_dro_Dro.droPoint(_render.ctx,postite_geom__$CoolPoint_CoolPoint_$Impl_$._new(_g12.x,_g12.y,30),postite_dro__$Couleur_Couleur_$Impl_$.fromString("#00AAFF"));
			break;
		case 1:
			break;
		}
...
postite_geom_GeomFilters.boundingBox = function(points) {
	var minX = Infinity;
	var maxX = -Infinity;
	var minY = Infinity;
	var maxY = -Infinity;
...
postite_geom_GeomFilters.scaleTo = function(points,size) {
	var B = postite_geom_GeomFilters.boundingBox(points);
	var newpoints = [];
	var _g = 0;
	var _g1 = points;
	while(_g < _g1.length) {
...
postite_geom_GeomFilters.clipOff = function(segs,scale) {
	...
	var ints = [];
	var result = new Array(segs.length);
	var _g = 0;
	var _g1 = segs.length;
	while(_g < _g1) {
		var i = _g++;
...
postite_geom_GeomFilters.isClosedAt = function(pts,simplify) {
	if(simplify == null) {
		simplify = true;
	}
	if(simplify) {
		...
		var begin_x = p1.x;
		var begin_y = p1.y;
		var begin_dx = p2.x - begin_x;
		var begin_dy = p2.y - begin_y;
		var p11 = simplepts[simplepts.length - 1];
		var p21 = simplepts[simplepts.length - 2];
		var end_x = p11.x;
		var end_y = p11.y;
		var end_dx = p21.x - end_x;
		var end_dy = p21.y - end_y;
		var pt = null;
		var inter;
		var p = postite_geom__$CoolPoint_Point_$Impl_$._new(begin_x,begin_y);
		var p12 = postite_geom__$CoolPoint_Point_$Impl_$._new(begin_x + begin_dx,begin_y + begin_dy);
		if((end_dx * (p.y - end_y) - end_dy * (p.x - end_x)) * (end_dx * (p12.y - end_y) - end_dy * (p12.x - end_x)) > 0) {
			inter = null;
		} else {
			var u = (end_dx * (begin_y - end_y) - end_dy * (begin_x - end_x)) / (end_dy * begin_dx - end_dx * begin_dy);
			if(u < 0 || u > 1) {
				inter = null;
			} else {
				pt = postite_geom__$CoolPoint_Point_$Impl_$._new();
				pt.x = begin_x + u * begin_dx;
				pt.y = begin_y + u * begin_dy;
				inter = pt;
			}
		}
		if(inter != null) {
			return haxe_ds_Option.Some(inter);
		}
...
postite_geom_Geste.prototype = {
	Recognize: function(points,useProtractor) {
		...
		var b = Infinity;
		console.log("src/postite/geom/Geste.hx:38:",this.Unistrokes.length);
		var u = -1;
		var _g = 0;
...
postite_geom_units__$Degree_Degree_$Impl_$.__name__ = true;
...
postite_geom_units__$Radian_Radian_$Impl_$.__name__ = true;
...
app_MockFlock.offseting = true;
app_MockFlock.bounding = true;
