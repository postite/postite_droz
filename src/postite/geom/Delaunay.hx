...
class DelaunayTriangle  {
	public var p1:CoolPoint;
	public var p2:CoolPoint;
	public var p3:CoolPoint;
...
class Delaunay {
	...
	public static function triangulate( CoolPoints:Array<CoolPoint> ) {
		...
		var xmin = CoolPoints[0].x;
		var ymin = CoolPoints[0].y;
		var xmax = xmin;
		var ymax = ymin;

		for( pt in CoolPoints ) {
		...
		// This is a triangle which encompasses all the sample CoolPoints.
		// The supertriangle coordinates are added to the end of the
		// vertex list. The supertriangle is the first triangle in
		// the triangle list.


		var p0 = new CoolPoint( xmid - 2 * dmax, ymid - dmax );
		var p1 = new CoolPoint( xmid, ymid + 2 * dmax );
		var p2 = new CoolPoint(xmid + 2 * dmax, ymid - dmax);
		CoolPoints.push(p0);
		CoolPoints.push(p1);
		CoolPoints.push(p2);

		var triangles = [];
		triangles.push( new DelaunayTriangle( CoolPoints[ nv ], CoolPoints[ nv + 1 ], CoolPoints[ nv + 2 ] ) ); //SuperTriangle placed at index 0

		// Include each CoolPoint one at a time into the existing mesh
		for( i in 0...nv ) {

			var DelaunayEdges = [];

			// Set up the DelaunayEdge buffer.
			// If the CoolPoint (vertex(i).x,vertex(i).y) lies inside the circumcircle then the
			...
			while( ++j < triangles.length ) {
				if ( InCircle( CoolPoints[i], triangles[j].p1, triangles[j].p2, triangles[j].p3 ) ) {
			...
			if( i >= nv ) continue; //In case we the last duplicate CoolPoint we removed was the last in the array



			// Remove duplicate DelaunayEdges
			// Note: if all triangles are specified anticlockwise then all
			// interior DelaunayEdges are opposite CoolPointing in direction.
			...
			// Form new triangles for the current CoolPoint
			...
			while( ++j < DelaunayEdges.length ) {
				if (triangles.length >= trimax )
					return null;
				triangles.push( new DelaunayTriangle( DelaunayEdges[ j ].p1, DelaunayEdges[ j ].p2, CoolPoints[ i ] ));
			}
	...
	static inline function InCircle( p:CoolPoint, p1:CoolPoint, p2:CoolPoint, p3:CoolPoint ){
